...
exports.handler = async (event, context) => {
  ...
  if (externalProvider === 'EXTERNAL_PROVIDER') {
    ...
    if (event.request.userAttributes.sub) {
      ...
      console.log('Success: user params', JSON.stringify(paramsInstatoolzPostConfirmation));
    ...
    if (event.request.userAttributes.sub) {
      ...
      try {
        ...
        const getUserCognito = await cognitoidentityserviceprovider
          ...
          .then((data) => {
            ...
            data?.UserAttributes.forEach((item) => (obj[`${item.Name}`] = item.Value));
            cognitoUser = { ...obj };
            return data;
          })
          .catch((err) => {
      ...
      const group =
        cognitoUser && cognitoUser['custom:group'] && cognitoUser['custom:group'] === 'business'
          ? 'Business'
          : 'User';
